...
const Referrals: React.FC = () => {
    ...
    const toggleLevel = (level: number) => {
        setCollapsedLevels(prev => { const newSet = new Set(prev); if (newSet.has(level)) newSet.delete(level); else newSet.add(level); return newSet; });
    }
    ...
    const fullGenealogyTree = useMemo(() => {
    ...
    const levelViewData = useMemo(() => {
        ...
        traverse(fullGenealogyTree);
        return levels;
    }, [fullGenealogyTree]);

    const networkStats = useMemo(() => {
        if (!currentUser) return { totalReferrals: 0, activeMembers: 0, earnings: 0, volume: 0 };
        ...
        const traverse = (nodes: GenealogyNode[]) => {
            nodes.forEach(node => {
                totalReferrals++;
                const isActive = node.user.activePlans?.some(p => p.planId === selectedPlanId);
                if (isActive) {
                    activeMembers++;
        ...
        return { totalReferrals, activeMembers, earnings: totalEarnings, volume };
    }, [fullGenealogyTree, transactions, currentUser, selectedPlanId]);


    const renderTreeNode = (node: GenealogyNode) => {
        const isCollapsed = collapsedNodes.has(node.user._id);
        const hasChildren = node.children.length > 0;
        const isActiveInPlan = node.user.activePlans?.some(p => p.planId === selectedPlanId);
        const commission = isActiveInPlan ? getCommissionFromReferralForPlan(node.user._id, selectedPlanId) : 0;
        const investment = isActiveInPlan ? getReferralInvestment(node.user, selectedPlanId) : 0;
        const joinDate = isActiveInPlan ? node.user.activePlans?.find(p => p.planId === selectedPlanId)?.purchaseDate : node.user.registrationDate;
        ...
        return (
            <li key={node.user._id} className="relative pl-6 py-2">
                ...
                <div className={`relative flex flex-col md:flex-row md:items-center bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 shadow-sm hover:shadow-md transition-all duration-200 group ${!isActiveInPlan && 'opacity-60 grayscale-[50%]'}`}>
                    <div className="flex items-center flex-grow space-x-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400">@{node.user.username} • {isActiveInPlan ? `Joined ${new Date(joinDate || Date.now()).toLocaleDateString()}` : `Registered ${new Date(joinDate || Date.now()).toLocaleDateString()}`}</p>
                        </div>
                    </div>
                    <div className="mt-4 md:mt-0 flex items-center justify-between md:justify-end space-x-6 pl-16 md:pl-0 border-t md:border-t-0 border-gray-100 dark:border-gray-700 pt-3 md:pt-0">
                        {isActiveInPlan ? (
                            ...
                            <div className="text-right text-xs italic text-gray-400 dark:text-gray-500">Inactive in Plan</div>
                        )}
                    </div>
                </div>
        ...
        return (
            <div className="space-y-4">
                {levels.map(level => {
                    ...
                    const activeMembers = members.filter(u => u.activePlans?.some(p => p.planId === selectedPlanId));
                    const isLevelCollapsed = !collapsedLevels.has(level);
                    const earnings = activeMembers.reduce((sum, u) => sum + getCommissionFromReferralForPlan(u._id, selectedPlanId), 0);
                    const volume = activeMembers.reduce((sum, u) => sum + getReferralInvestment(u, selectedPlanId), 0);
                            ...
                            <button onClick={() => toggleLevel(level)} className="w-full p-4 flex justify-between items-center hover:bg-gray-50 dark:hover:bg-gray-700/50 rounded-t-lg">
                                <div className="flex items-center space-x-4">
                                    ...
                                    <div className="text-left text-sm text-gray-600 dark:text-gray-400 hidden sm:flex gap-4">
                                        <span><span className="font-bold">{activeMembers.length}</span> Active / {members.length} Total</span>
                            ...
                            {!isLevelCollapsed && (
                                <div className="p-4 border-t dark:border-gray-700 animate-fade-in-down">
                                    <div className="space-y-2">
                                        {members.map(member => {
                                            const isActive = member.activePlans?.some(p => p.planId === selectedPlanId);
                                            const commission = isActive ? getCommissionFromReferralForPlan(member._id, selectedPlanId) : 0;
                                            const investment = isActive ? getReferralInvestment(member, selectedPlanId) : 0;
                                            const date = isActive ? member.activePlans?.find(p => p.planId === selectedPlanId)?.purchaseDate : member.registrationDate;

                                            return (
                                                <div key={member._id} className={`grid grid-cols-12 gap-2 items-center p-2 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700/50 text-xs ${!isActive && 'opacity-60'}`}>
                                                    ...
                                                    <div className="col-span-2 text-gray-500">{new Date(date || Date.now()).toLocaleDateString()}</div>
                                                    <div className="col-span-2 text-right">{isActive ? `$${investment.toFixed(0)}` : '-'}</div>
                                                    <div className="col-span-2 text-right font-bold text-green-600">{isActive ? `+$${commission.toFixed(2)}` : '-'}</div>
                                                </div>
                                            )
                                        })}
                                    </div>
    ...
    const renderEmptyState = () => (
        <div className="flex flex-col items-center justify-center h-full py-12">
            ...
            <p className="text-gray-500 dark:text-gray-400 max-w-md mx-auto mt-2 text-center">
                You haven't referred anyone yet. Share your link to start building your network!
            </p>
    ...
    if (uniqueActivePlans.length === 0 && fullGenealogyTree.length === 0) {
        return (
    ...
    // Fallback to first plan if none selected (edge case if user has no plans but has referrals)
            ...
            <div className="bg-gray-50 dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
                <div className="p-5 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-xs text-gray-500">Interactive view of your team</p>
                    </div>
                ...
                <div className="p-8 overflow-x-auto min-h-[400px]">
                    {viewMode === 'tree' ? (
                        fullGenealogyTree.length > 0 ? (
                            <ul className="space-y-4 min-w-[600px]">{fullGenealogyTree.map(node => renderTreeNode(node))}</ul>
                        ) : (
                             renderEmptyState()
                        )
                    ) : renderLevelView()}
