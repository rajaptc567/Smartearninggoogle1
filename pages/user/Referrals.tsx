...
const Referrals: React.FC = () => {
    ...
    const { genealogyTree, directEarners, indirectEarners, inactiveReferrals, networkStats, allNodes } = useMemo(() => {
        if (!currentUser) return { genealogyTree: [], directEarners: [], indirectEarners: [], networkStats: { totalReferrals: 0, activeMembers: 0, earnings: 0, directEarnings: 0, indirectEarnings: 0 }, allNodes: [] };

        // FIXED: Added cycle detection and depth limit
        const visited = new Set<string>();
        const buildFullTree = (sponsorUsername: string, level: number, depth: number = 0): GenealogyNode[] => {
            if (!sponsorUsername) return [];
            const normalized = sponsorUsername.toLowerCase();
            if (visited.has(normalized) || depth > 10) return [];
            visited.add(normalized);
            
            const directReferrals = users.filter(u => u.sponsor && u.sponsor.toLowerCase() === normalized);
            return directReferrals.map(child => ({
                user: child,
                children: buildFullTree(child.username, level + 1, depth + 1),
                level
            }));
        };
        ...
        nodesList.forEach(node => {
            ...
            if (earned > 0) {
                ...
                if (!node.user.activePlans || node.user.activePlans.length === 0) inactiveList.push(node);
            }
        });
    ...
    const ReferralCardContent: React.FC<{
        ...
        let earned = 0; let held = 0; let breakdown: { reason: string, planId?: string, planName?: string, amount: number }[] = [];
        let earningSourcePlanId: string | undefined; let commissionSourcePlans: string[] = [];
        if (isHeldView) {
                            ...
                            <div className="bg-yellow-50 dark:bg-yellow-900/10 p-2 rounded border border-yellow-100 dark:border-yellow-800 space-y-2 w-full sm:w-auto">
                                {breakdown.map((item, idx) => (
                                    <div key={idx} className="flex flex-col items-end gap-1">
                                        <div className="text-sm flex items-center justify-end gap-2"><span className="text-yellow-700 dark:text-yellow-400 font-medium text-xs">{item.reason}:</span><span className="font-bold text-yellow-800 dark:text-yellow-300">{formatCurrency(item.amount, currentUser?.currency)}</span></div>
    ...
    const renderTreeNode = (node: GenealogyNode & { isSkipped?: boolean }) => {
        ...
        return (
            <li key={node.user._id} className="relative pl-4 sm:pl-6 pt-2">
                ...
                <div className="mb-2"><ReferralCardContent node={node} toggleNode={toggleNode} isCollapsed={isCollapsed} hasChildren={hasChildren} isTree={true} /></div>
        ...
        <div className="space-y-6 max-w-6xl mx-auto">
            ...
            {uniqueActivePlans.length > 0 ? (
                    ...
                    {uniqueActivePlans.map(plan => {
                        ...
                        return (
                            <button 
                                ...
                                className={`flex-1 min-w-[140px] py-2 px-4 rounded-md text-sm font-medium transition-all whitespace-nowrap border ${isActive ? 'bg-blue-50 border-blue-200 text-blue-700 dark:bg-blue-900/30 dark:border-blue-800 dark:text-blue-300 shadow-sm' : 'bg-transparent border-transparent text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700'}`}
                            >
                                {plan.planName}
                            </button>
                        )
                ...
                <div className="bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-700 p-4 rounded-lg flex items-center justify-between">
                    <div className="flex items-center gap-3"><span className="text-2xl">⚠️</span><div><h3 className="font-bold text-yellow-800 dark:text-yellow-200">No Active Plans</h3><p className="text-xs text-yellow-700 dark:text-yellow-300">Purchase a plan to start earning commissions.</p></div></div><Button size="sm" onClick={() => navigate('/member/plans')}>Buy Plan</Button>
                </div>
            )}
            ...
            <div className="bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden min-h-[500px]">
                <div className="p-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex flex-wrap gap-2">
                    ...
                    <button onClick={() => setViewMode('inactive')} className={`px-4 py-2 text-xs font-bold rounded-full transition-colors ${viewMode === 'inactive' ? 'bg-red-600 text-white' : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300'}`}>Inactive Member ({inactiveReferrals.length})</button>
                </div>
                <div className="p-4 md:p-6 overflow-x-auto">
                    ...
                    {viewMode === 'held' && <div className="space-y-3">{globalHeldData.referrals.length > 0 ? globalHeldData.referrals.map(user => <ReferralCardContent key={user._id} node={{user}} toggleNode={() => {}} isCollapsed={false} hasChildren={false} isTree={false} isHeldView={true} />) : <div className="flex flex-col items-center justify-center py-12 text-gray-400"><p>No held commissions.</p></div>}</div>}
                    {viewMode === 'all' && <div className="space-y-3">{allNodes.length > 0 ? allNodes.map(node => <ReferralCardContent key={node.user._id} node={node} toggleNode={() => {}} isCollapsed={false} hasChildren={false} isTree={false} isAllView={true} />) : <div className="flex flex-col items-center justify-center py-12 text-gray-400"><p>No referrals found.</p></div>}</div>}
            ...
            {isSponsorModalOpen && selectedSponsor && selectedReferralForSponsorModal && (
                <Modal isOpen={isSponsorModalOpen} onClose={() => setIsSponsorModalOpen(false)}>
                    <div className="p-6 max-w-md text-center">
                        ...
                        <div className="mt-6 space-y-2"><button onClick={handleLocateSponsor} className="w-full py-2 px-4 bg-purple-600 text-white rounded hover:bg-purple-700 transition-colors">Locate {selectedSponsor.username} in Tree</button><Button variant="secondary" onClick={() => setIsSponsorModalOpen(false)} className="w-full">Close</Button></div>
                    </div>
                </Modal>
            )}
        </div>
