...
const Referrals: React.FC = () => {
    ...
    // Robust matching for hold positions using the new flag
    const isTransactionHoldPosition = (t: Transaction) => {
        // We now prioritize the explicit boolean flag from the backend
        ...
        const hasKeywords = desc.includes('position') || desc.includes('reserved') || desc.includes('upgrade');
    ...
    const { sponsorEarnings, displaySourcePlanName, planToView, isLinkedPlanEquivalent } = sponsorModalDetails;
        ...
        <div className="space-y-6 max-w-6xl mx-auto">
            ...
            {(viewMode !== 'held' && viewMode !== 'all') && (
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                    ...
                    <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700">
                        <p className="text-xs text-amber-600 uppercase tracking-wider mb-1">Held for Upgrade</p>
                        <h3 className="text-2xl font-bold text-amber-600">{formatCurrency(currentUser.heldBalance, currentUser.currency)}</h3>
                        <p className="text-xs text-gray-400 mt-1">Reserved Funds</p>
                    </div>
                    <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700">
                    ...
                    <div className="p-6 max-w-md text-center">
                        ...
                        <div className="bg-gray-50 dark:bg-gray-700/50 p-5 rounded-xl text-left border border-gray-100 dark:border-gray-600 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4 bg-white dark:bg-gray-800 p-3 rounded-lg border border-gray-200 dark:border-gray-600">
                                <div><p className="text-xs text-gray-500 uppercase font-bold mb-1">Earning Source Plan</p><p className="font-bold text-gray-900 dark:text-white text-sm">{displaySourcePlanName}</p></div>
                                <div className="border-l pl-3 dark:border-gray-600"><p className="text-xs text-blue-500 uppercase font-bold mb-1">Your Linked Plan</p><p className="font-bold text-blue-700 dark:text-blue-300 text-sm">{selectedPlanDetails?.name || 'None'}</p>{isLinkedPlanEquivalent && <p className="text-[10px] bg-indigo-50 text-indigo-700 px-1.5 py-0.5 mt-1 rounded border border-indigo-100 dark:bg-indigo-900/30 dark:text-indigo-300 dark:border-indigo-800">(Equivalent to {displaySourcePlanName})</p>}</div>
                            </div>
