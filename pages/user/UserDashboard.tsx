...
const UserDashboard: React.FC = () => {
    const { state } = useData();
    const { currentUser, deposits, withdrawals, transactions, users } = state;
    ...
    const stats = useMemo(() => {
        ...
        const approvedCommissions = userTransactions.filter(t => t.type === 'Commission' && t.status === 'Approved' && t.currency === selectedCurrency);
        ...
        const activePlanValue = (currentUser.activePlans || []).filter(p => p.currency === selectedCurrency).reduce((sum, p) => sum + p.price, 0);
        ...
        return {
            totalDeposits: deposits.filter(d => d.userId === currentUser._id && d.status === Status.Approved && d.currency === selectedCurrency).reduce((sum, d) => sum + d.amount, 0),
            totalWithdrawals: withdrawals.filter(w => w.userId === currentUser._id && w.status === Status.Paid && w.currency === selectedCurrency).reduce((sum, w) => sum + w.finalAmount, 0),
            totalCommission,
            directCommission,
            indirectCommission,
            pendingCommission: userTransactions.filter(t => t.type === 'Commission' && t.status === 'Pending' && t.currency === selectedCurrency).reduce((sum, t) => sum + t.amount, 0),
            monthlyEarnings: approvedCommissions.filter(t => t.date >= firstDayOfMonth).reduce((sum, t) => sum + t.amount, 0),
            activePlanValue,
            activePlansDisplay
        };
    }, [userTransactions, deposits, withdrawals, currentUser, selectedCurrency]);
    ...
    return (
        <div className="space-y-8">
            <div>
                ...
                <p className="text-gray-500 dark:text-gray-400 mt-1">Here's a summary of your account activity in <span className="font-bold">{selectedCurrency}</span>.</p>
            </div>
            ...
            <div className="relative">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {visibleWidgets.balance && <StatCard title="Available Balance" value={formatCurrency(currentUser.walletBalances[selectedCurrency] || 0)} icon={<WalletIcon />} color="bg-blue-500" />}
                    {visibleWidgets.deposits && <StatCard title="Total Deposits" value={formatCurrency(stats.totalDeposits)} icon={<DepositIcon />} color="bg-sky-500" />}
                    {visibleWidgets.commission && <StatCard title="Total Commission" value={formatCurrency(stats.totalCommission)} icon={<EarningsIcon />} color="bg-green-500" />}
                    {visibleWidgets.withdrawals && <StatCard title="Total Withdrawals" value={formatCurrency(stats.totalWithdrawals)} icon={<WithdrawalIcon />} color="bg-red-500" />}
                    {visibleWidgets.pending && <StatCard title="Pending Commission" value={formatCurrency(stats.pendingCommission)} icon={<ClockIcon />} color="bg-yellow-500" />}
                    ...
                    {visibleWidgets.monthly && <StatCard title="Earnings This Month" value={formatCurrency(stats.monthlyEarnings)} icon={<EarningsIcon />} color="bg-teal-500" />}
                    {visibleWidgets.plan && <StatCard title="Active Plans Value" value={formatCurrency(stats.activePlanValue)} icon={<PlanIcon />} color="bg-pink-500" />}
                </div>
            </div>
                ...
                <Table headers={['ID', 'Type', 'Amount', 'Status', 'Date', 'Description']}>
                    {recentTransactions.map((tx: Transaction) => (
                         <tr key={tx._id} className="text-gray-700 dark:text-gray-400">
                            ...
                            <td className={`px-4 py-3 text-sm font-semibold ${tx.amount > 0 ? 'text-green-600' : 'text-red-600'}`}>{tx.amount > 0 ? '+' : ''}{formatCurrency(tx.amount, tx.currency)}</td>
