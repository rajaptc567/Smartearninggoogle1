...
const Settings: React.FC = () => {
  ...
  // Investment Module Allowlist States (When Disabled Bypass)
  const [invUserSearchQuery, setInvUserSearchQuery] = useState('');
  const [invUserStatusFilter, setInvUserStatusFilter] = useState<'all' | 'active_plan' | 'no_plan'>('all');
  const [invUserRoleFilter, setInvUserRoleFilter] = useState<'all' | 'user' | 'admin' | 'finance' | 'support'>('all');
  ...
  useEffect(() => {
    ...
    setLocalSettings(prev => ({
        ...
        investmentAllowActivePlanUsersWhenDisabled: settings.investmentAllowActivePlanUsersWhenDisabled === true,
        investmentAllowedUserIds: Array.isArray(settings.investmentAllowedUserIds) ? settings.investmentAllowedUserIds : [],
        investmentAllowedUsernames: Array.isArray(settings.investmentAllowedUsernames) ? settings.investmentAllowedUsernames : [],
  ...
  const filteredUsersForInvestment = React.useMemo(() => {
      return (users || []).filter(u => {
          const matchesSearch = !invUserSearchQuery || 
              (u.username && u.username.toLowerCase().includes(invUserSearchQuery.toLowerCase())) ||
              (u.email && u.email.toLowerCase().includes(invUserSearchQuery.toLowerCase())) ||
              (u.fullName && u.fullName.toLowerCase().includes(invUserSearchQuery.toLowerCase()));
          
          const hasActivePlan = u.activePlans && u.activePlans.length > 0;
          const matchesStatus = invUserStatusFilter === 'all' ||
              (invUserStatusFilter === 'active_plan' && hasActivePlan) ||
              (invUserStatusFilter === 'no_plan' && !hasActivePlan);
          
          const matchesRole = invUserRoleFilter === 'all' || u.role === invUserRoleFilter;
          
          return matchesSearch && matchesStatus && matchesRole;
      });
  }, [users, invUserSearchQuery, invUserStatusFilter, invUserRoleFilter]);

  const toggleInvestmentAllowedUser = (user: { _id: string; username: string }) => {
      const currentAllowedIds = localSettings.investmentAllowedUserIds || [];
      const currentAllowedUsernames = localSettings.investmentAllowedUsernames || [];
      const isCurrentlyAllowed = currentAllowedIds.includes(user._id);

      let newAllowedIds: string[];
      let newAllowedUsernames: string[];

      if (isCurrentlyAllowed) {
          newAllowedIds = currentAllowedIds.filter(id => id !== user._id);
          newAllowedUsernames = currentAllowedUsernames.filter(u => u !== user.username);
      } else {
          newAllowedIds = [...currentAllowedIds, user._id];
          newAllowedUsernames = Array.from(new Set([...currentAllowedUsernames, user.username]));
      }

      setLocalSettings(prev => ({
          ...prev,
          investmentAllowedUserIds: newAllowedIds,
          investmentAllowedUsernames: newAllowedUsernames
      }));
      setIsDirty(true);
  };

  const handleBulkEnableInvestmentFiltered = () => {
      const currentAllowedIds = localSettings.investmentAllowedUserIds || [];
      const currentAllowedUsernames = localSettings.investmentAllowedUsernames || [];
      const filteredIds = filteredUsersForInvestment.map(u => u._id);
      const filteredUsernames = filteredUsersForInvestment.map(u => u.username).filter(Boolean);

      const newAllowedIds = Array.from(new Set([...currentAllowedIds, ...filteredIds]));
      const newAllowedUsernames = Array.from(new Set([...currentAllowedUsernames, ...filteredUsernames]));
  ...
  const handleBulkDisableInvestmentFiltered = () => {
      const currentAllowedIds = localSettings.investmentAllowedUserIds || [];
      const currentAllowedUsernames = localSettings.investmentAllowedUsernames || [];
      const filteredIds = filteredUsersForInvestment.map(u => u._id);
      const filteredUsernames = filteredUsersForInvestment.map(u => u.username).filter(Boolean);

      const newAllowedIds = currentAllowedIds.filter(id => !filteredIds.includes(id));
      const newAllowedUsernames = currentAllowedUsernames.filter(u => !filteredUsernames.includes(u));
                ...
                <div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className={`p-5 rounded-2xl border transition-all ${
                                ...
                                <div className="space-y-1.5 max-w-2xl">
                                    <div className="flex items-center gap-2.5">
                                        ...
                                        <span className={`px-2.5 py-0.5 text-[11px] font-black rounded-full uppercase tracking-wider ${
                                            ...
                                            {(localSettings.investmentModuleEnabled ?? true) ? '● Status: ON (Active for All)' : '○ Status: OFF (Disabled / Whitelist Only)'}
                                        </span>
                                    </div>
                                    <p className="text-xs text-gray-600 dark:text-gray-300 leading-relaxed">
                                        {(localSettings.investmentModuleEnabled ?? true)
                                            ...
                                            : 'The Investment Module is hidden from general users by default. Work & Earn remains 100% active and operational. You can configure granular exceptions below to allow specific members or active plan holders to view it while disabled.'}
                                    </p>
                                </div>
                            ...
                            {/* GRANULAR BYPASS & ALLOWLIST CONTROLS */}
                            <div className="mt-5 pt-4 border-t border-gray-200 dark:border-gray-700/60 space-y-4">
                                <div className="flex items-center justify-between">
                                    <h5 className="text-xs font-bold uppercase tracking-wider text-gray-700 dark:text-gray-300 flex items-center gap-1.5">
                                        <span>🛡️</span>
                                        <span>Access Exceptions (When Disabled)</span>
                                    </h5>
                                    {!(localSettings.investmentModuleEnabled ?? true) && (
                                        <span className="text-[10px] font-bold text-amber-500 bg-amber-500/10 border border-amber-500/20 px-2 py-0.5 rounded-full">
                                            Active Rules Applied While Module is OFF
                                        </span>
                                    )}
                                </div>

                                {/* Exception 1: Active Plans Bypass */}
                                <div className="flex items-center justify-between p-3.5 bg-gray-50/80 dark:bg-gray-800/80 rounded-xl border dark:border-gray-700/60">
                                    <div className="space-y-0.5 max-w-xl">
                                        <label htmlFor="investmentAllowActivePlanUsers" className="text-xs font-bold text-gray-800 dark:text-gray-200 flex items-center gap-2 cursor-pointer">
                                            <span>⭐ Allow Members with Active Plans</span>
                                            <span className={`text-[10px] px-2 py-0.2 rounded-full font-bold ${
                                                localSettings.investmentAllowActivePlanUsersWhenDisabled
                                                    ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30'
                                                    : 'bg-gray-500/20 text-gray-400'
                                            }`}>
                                                {localSettings.investmentAllowActivePlanUsersWhenDisabled ? 'Enabled' : 'Disabled'}
                                            </span>
                                        </label>
                                        <p className="text-[11px] text-gray-500 dark:text-gray-400">
                                            Automatically permit users with at least one active investment plan to see and access the Investment Module even when the master switch is OFF.
                                        </p>
                                    </div>
                                    <div className="relative inline-block w-12 h-6 transition duration-200 ease-in-out shrink-0 ml-3">
                                        <input
                                            id="investmentAllowActivePlanUsers"
                                            type="checkbox"
                                            ...
                                            checked={localSettings.investmentAllowActivePlanUsersWhenDisabled ?? false}
                                            onChange={() => {
                                                setLocalSettings(prev => ({
                                                    ...prev,
                                                    investmentAllowActivePlanUsersWhenDisabled: !prev.investmentAllowActivePlanUsersWhenDisabled
                                                }));
                                                setIsDirty(true);
                                            }}
                                        />
                                        <label
                                            htmlFor="investmentAllowActivePlanUsers"
                                            className={`toggle-label block overflow-hidden h-6 rounded-full cursor-pointer ${localSettings.investmentAllowActivePlanUsersWhenDisabled ? 'bg-blue-600' : 'bg-gray-400 dark:bg-gray-600'}`}
                                        ></label>
                                    </div>
                                </div>

                                {/* Exception 2: Manual Member Whitelist */}
                                <div className="p-4 bg-gray-50/80 dark:bg-gray-800/80 rounded-xl border dark:border-gray-700/60 space-y-3">
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                                        <div>
                                            <h6 className="text-xs font-bold text-gray-800 dark:text-gray-200 flex items-center gap-1.5">
                                                <span>👤</span>
                                                <span>Manually Whitelisted Members</span>
                                            </h6>
                                            <p className="text-[11px] text-gray-500 dark:text-gray-400">
                                                Explicitly select users who can access the Investment Module at all times, even when disabled.
                                            </p>
                                        </div>
                                        <span className="bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-300 text-xs px-2.5 py-0.5 rounded-full font-bold shrink-0 self-start sm:self-auto">
                                            {(localSettings.investmentAllowedUserIds || []).length} Whitelisted Users
                                        </span>
                                    </div>

                                    {/* Whitelisted tags preview */}
                                    {(localSettings.investmentAllowedUserIds || []).length > 0 && (
                                        <div className="flex flex-wrap gap-1.5 p-2 bg-white dark:bg-gray-900 rounded-lg border dark:border-gray-700 max-h-24 overflow-y-auto">
                                            {(localSettings.investmentAllowedUserIds || []).map((userId: string) => {
                                                const u = users.find(usr => usr._id === userId);
                                                const displayName = u ? (u.username || u.fullName || u.email) : userId;
                                                return (
                                                    <span
                                                        key={userId}
                                                        className="inline-flex items-center gap-1 text-[11px] font-semibold bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded-md border border-blue-200 dark:border-blue-800"
                                                    >
                                                        <span>{displayName}</span>
                                                        <button
                                                            type="button"
                                                            onClick={() => toggleInvestmentAllowedUser({ _id: userId, username: u?.username || '' })}
                                                            className="text-blue-400 hover:text-rose-500 ml-0.5 font-bold"
                                                            title="Remove user"
                                                        >
                                                            ✕
                                                        </button>
                                    ...
                                    {/* Filters for Whitelist selector */}
                                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 pt-1">
                                        <div className="space-y-1">
                                            <label className="text-[10px] uppercase font-bold text-gray-400">Search Members</label>
                                            <input
                                                type="text"
                                                value={invUserSearchQuery}
                                                onChange={(e) => setInvUserSearchQuery(e.target.value)}
                                                className="w-full text-xs p-2 rounded-lg border dark:bg-gray-900 dark:border-gray-700"
                                                placeholder="Username, email, name..."
                                            />
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-[10px] uppercase font-bold text-gray-400">Plan Status</label>
                                            <select
                                                value={invUserStatusFilter}
                                                onChange={(e) => setInvUserStatusFilter(e.target.value as any)}
                                                className="w-full text-xs p-2 rounded-lg border dark:bg-gray-900 dark:border-gray-700"
                                            >
                                                <option value="all">All Members</option>
                                                <option value="active_plan">Has Active Plan</option>
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <select
                                                value={invUserRoleFilter}
                                                onChange={(e) => setInvUserRoleFilter(e.target.value as any)}
                                                className="w-full text-xs p-2 rounded-lg border dark:bg-gray-900 dark:border-gray-700"
                                            >
                                    ...
                                    <div className="flex items-center justify-between pt-1">
                                        <span className="text-[11px] text-gray-500 dark:text-gray-400">
                                            Showing {filteredUsersForInvestment.length} matching member(s)
                                        </span>
                                        <div className="flex items-center gap-2">
                                            <button
                                                type="button"
                                                onClick={handleBulkEnableInvestmentFiltered}
                                                className="text-[11px] px-2.5 py-1 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-bold transition-colors"
                                            >
                                                Allow Filtered ({filteredUsersForInvestment.length})
                                            </button>
                                            <button
                                                type="button"
                                                onClick={handleBulkDisableInvestmentFiltered}
                                                className="text-[11px] px-2.5 py-1 bg-gray-200 dark:bg-gray-700 hover:bg-rose-600 hover:text-white text-gray-700 dark:text-gray-300 rounded-lg font-bold transition-colors"
                                            >
                                                Revoke Filtered
                                            </button>
                                        </div>
                                    ...
                                    {/* User items scroll list */}
                                    <div className="border dark:border-gray-700 rounded-lg divide-y dark:divide-gray-700 max-h-52 overflow-y-auto bg-white dark:bg-gray-900">
                                        {filteredUsersForInvestment.length === 0 ? (
                                            <div className="p-4 text-center text-xs text-gray-400">
                                                No members found matching your search.
                                            </div>
                                        ) : (
                                            filteredUsersForInvestment.slice(0, 50).map(u => {
                                                const isAllowed = (localSettings.investmentAllowedUserIds || []).includes(u._id);
                                                const hasActivePlan = u.activePlans && u.activePlans.length > 0;
                                                return (
                                                    <div
                                                        key={u._id}
                                                        onClick={() => toggleInvestmentAllowedUser({ _id: u._id, username: u.username })}
                                                        ...
                                                        <div className="flex items-center gap-2.5">
                                                            ...
                                                            <div>
                                                                <div className="font-bold text-gray-800 dark:text-gray-200 flex items-center gap-1.5">
                                                                    <span>{u.username || u.fullName || 'User'}</span>
                                                                    {hasActivePlan && (
                                                                        <span className="text-[9px] bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 px-1.5 py-0.2 rounded font-semibold">
                                                                            Active Plan ({u.activePlans.length})
                                                                        </span>
                                                                    )}
                                                                </div>
                                                                <div className="text-[10px] text-gray-400">{u.email || u._id}</div>
                                                            </div>
                                                        </div>
