...
import { Settings, DemoProfile, DemoActivityTemplate, Currency, countries } from '../types';
                    ...
                    {(localSettings.demoProfiles || []).map((profile) => (
                        <div key={profile._id} className="grid grid-cols-12 gap-4 items-center p-2 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700/50">
                            {editingProfileId === profile._id ? (
                                <>
                                    <div className="col-span-3"><input value={editedData.name} onChange={e => handleEditedDataChange('name', e.target.value)} className="w-full rounded-md dark:bg-gray-700 text-sm py-1" /></div>
                                    <div className="col-span-3">
                                        <select value={editedData.country} onChange={e => handleEditedDataChange('country', e.target.value)} className="w-full rounded-md dark:bg-gray-700 text-sm py-1">
                                            {countries.map(c => <option key={c} value={c}>{c}</option>)}
                                        </select>
                                    </div>
                                    <div className="col-span-3">
                                        <select value={editedData.currency} onChange={e => handleEditedDataChange('currency', e.target.value as Currency)} className="w-full rounded-md dark:bg-gray-700 text-sm py-1">
                                            <option value="USD">USD</option><option value="EUR">EUR</option><option value="PKR">PKR</option>
                                        </select>
                                    </div>
                                    <div className="col-span-3 flex gap-2 justify-end">
                                        <Button size="sm" variant="success" onClick={handleUpdateProfile}>Save</Button>
                                        <Button size="sm" variant="secondary" onClick={handleCancelEditProfile}>Cancel</Button>
                                    </div>
                                </>
                            ) : (
                                <>
                                    <div className="col-span-3 text-sm font-medium">{profile.name}</div>
                                    <div className="col-span-3 text-sm text-gray-600 dark:text-gray-400">{profile.country}</div>
                                    <div className="col-span-3"><span className="px-2 py-1 text-xs font-semibold rounded bg-gray-200 dark:bg-gray-700">{profile.currency}</span></div>
                                    <div className="col-span-3 flex gap-2 justify-end">
                                        <Button size="sm" variant="secondary" onClick={() => handleEditProfile(profile)}>Edit</Button>
                ...
                {/* Add New Template Form */}
                 <form onSubmit={handleAddTemplate} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end mb-6 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg border dark:border-gray-700">
                    <div className="md:col-span-6">
                        ...
                        <input value={newTemplate.template} onChange={e => setNewTemplate(p => ({ ...p, template: e.target.value }))} placeholder="{name} from {country}..." className="w-full rounded-md dark:bg-gray-700 text-sm py-2" required />
                    </div>
                     <div className="md:col-span-3">
                        <label className="text-sm font-medium">Type</label>
                        <select value={newTemplate.type} onChange={e => setNewTemplate(p => ({ ...p, type: e.target.value as any }))} className="w-full rounded-md dark:bg-gray-700 text-sm py-2" required>
                             <option value="joined">Joined</option><option value="deposit">Deposit</option><option value="withdrawal">Withdrawal</option><option value="transfer">Transfer</option><option value="plan">Plan Purchase</option>
                        </select>
                    </div>
                    <div className="md:col-span-3">
                        <Button type="submit" className="w-full py-2">Add Template</Button>
                    </div>
                </form>
                    ...
                    {(localSettings.demoActivityTemplates || []).map((template) => (
                        <div key={template._id} className="grid grid-cols-12 gap-4 items-center p-2 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700/50">
                            {editingTemplateId === template._id ? (
                                <>
                                    <div className="col-span-5"><input value={editedData.template} onChange={e => handleEditedDataChange('template', e.target.value)} className="w-full rounded-md dark:bg-gray-700 text-sm py-1" /></div>
                                    <div className="col-span-2">
                                        <select value={editedData.type} onChange={e => handleEditedDataChange('type', e.target.value)} className="w-full rounded-md dark:bg-gray-700 text-sm py-1">
                                            <option value="joined">Joined</option><option value="deposit">Deposit</option><option value="withdrawal">Withdrawal</option><option value="transfer">Transfer</option><option value="plan">Plan Purchase</option>
                                        </select>
                                    </div>
                                    <div className="col-span-2 flex justify-center"><input type="checkbox" checked={editedData.enabled} onChange={e => handleEditedDataChange('enabled', e.target.checked)} /></div>
                                    <div className="col-span-3 flex gap-2 justify-end">
                                        <Button size="sm" variant="success" onClick={handleUpdateTemplate}>Save</Button>
                                        <Button size="sm" variant="secondary" onClick={handleCancelEditTemplate}>Cancel</Button>
                                    </div>
                                </>
                            ) : (
                                <>
                                    ...
                                    <div className="col-span-2 flex justify-center">
                                         <label className="inline-flex items-center cursor-pointer">
                                            <input type="checkbox" checked={template.enabled} onChange={e => {
                                                const updatedTemplates = (localSettings.demoActivityTemplates || []).map(t => t._id === template._id ? {...t, enabled: e.target.checked} : t);
                                                setLocalSettings(prev => ({ ...prev, demoActivityTemplates: updatedTemplates }));
                                                setIsDirty(true);
                                            }} className="sr-only peer" />
                                            <div className="relative w-9 h-5 bg-gray-200 peer-focus:outline-none rounded-full peer dark:bg-gray-600 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
                                        </label>
                                    </div>
                                    <div className="col-span-3 flex gap-2 justify-end">
                                        <Button size="sm" variant="secondary" onClick={() => handleEditTemplate(template)}>Edit</Button>
