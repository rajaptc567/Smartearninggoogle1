    ...
    return (
        <div className="flex flex-col md:flex-row items-center justify-center gap-4">
            <svg viewBox="0 0 100 100" className="w-32 h-32 transform -rotate-90">
                {segments.map((segment, index) => (<circle key={index} cx="50" cy="50" r="40" fill="transparent" stroke={segment.color} strokeWidth="20" strokeDasharray={segment.dashArray} strokeDashoffset={segment.dashOffset} transform={`rotate(${segment.rotation} 50 50)`} />))}
            </svg>
            <div className="text-sm space-y-1">
                {data.map(item => (<div key={item.label} className="flex items-center"><span className="w-3 h-3 rounded-full mr-2" style={{ backgroundColor: item.color }}></span><span>{item.label}:</span><span className="font-semibold ml-1">{formatCurrency(item.value, currency as any)}</span></div>))}
            </div>
        </div>
    );
};
...
const UserDashboard: React.FC = () => {
    ...
    const [visibleWidgets, setVisibleWidgets] = useState({ balance: true, deposits: true, commission: true, withdrawals: true, pending: true, referrals: true, plan: true, monthly: true, breakdown: true });
    ...
    const stats = useMemo(() => {
        ...
        return { totalDeposits: deposits.filter(d => d.userId === currentUser._id && d.status === Status.Approved).reduce((sum, d) => sum + d.amount, 0), totalWithdrawals: withdrawals.filter(w => w.userId === currentUser._id && w.status === Status.Paid).reduce((sum, w) => sum + w.finalAmount, 0), totalCommission, directCommission, indirectCommission, pendingCommission: userTransactions.filter(t => t.type === 'Commission' && t.status === 'Pending').reduce((sum, t) => sum + t.amount, 0), monthlyEarnings: approvedCommissions.filter(t => t.date >= firstDayOfMonth).reduce((sum, t) => sum + t.amount, 0), activePlanCount: (currentUser.activePlans || []).length, activePlanValue: (currentUser.activePlans || []).reduce((sum, p) => sum + p.price, 0) };
    ...
    const networkBreakdown = useMemo(() => {
        const breakdown: { active: { [key: number]: number }, inactive: { [key: number]: number }, total: number } = { active: {}, inactive: {}, total: 0 };
        // FIXED: Added cycle detection and depth limit
        const visited = new Set<string>();
        const traverse = (sponsorUsername: string, level: number, depth: number = 0) => {
            if (!sponsorUsername) return;
            const normalized = sponsorUsername.toLowerCase();
            if (visited.has(normalized) || depth > 10) return;
            visited.add(normalized);
            
            const referrals = users.filter(u => u.sponsor && u.sponsor.toLowerCase() === normalized);
            if (referrals.length === 0) return;
            
            referrals.forEach(ref => {
                breakdown.total++;
                const isActive = ref.activePlans && ref.activePlans.length > 0;
                if (isActive) breakdown.active[level] = (breakdown.active[level] || 0) + 1;
                else breakdown.inactive[level] = (breakdown.inactive[level] || 0) + 1;
                traverse(ref.username, level + 1, depth + 1);
            });
        };
    ...
    const toggleWidget = (widget: keyof typeof visibleWidgets) => setVisibleWidgets(prev => ({ ...prev, [widget]: !prev[widget] }));
        ...
        return (
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 sm:p-6">
                <div className="flex items-center justify-between mb-4">
                    <div><p className="text-sm font-medium text-gray-500 dark:text-gray-400">Network Overview</p><p className="text-2xl font-semibold text-gray-800 dark:text-white">{networkBreakdown.total} Total Referrals</p></div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm mt-4 border-t dark:border-gray-700 pt-4">
                    <div><h4 className="font-semibold text-green-600 dark:text-green-400 mb-2">Active Referrals ({totalActive})</h4><ul className="space-y-1 text-gray-600 dark:text-gray-300">{Object.keys(networkBreakdown.active).map(level => (<li key={`active-${level}`} className="flex justify-between"><span>Level {level}:</span><span className="font-bold">{networkBreakdown.active[parseInt(level)]}</span></li>))}</ul></div>
                    <div><h4 className="font-semibold text-red-500 dark:text-red-400 mb-2">Inactive Referrals ({totalInactive})</h4><ul className="space-y-1 text-gray-600 dark:text-gray-300">{Object.keys(networkBreakdown.inactive).map(level => (<li key={`inactive-${level}`} className="flex justify-between"><span>Level {level}:</span><span className="font-bold">{networkBreakdown.inactive[parseInt(level)]}</span></li>))}</ul></div>
                </div>
            </div>
        );
    };
    ...
    return (
        <div className="space-y-8">
            <div className="flex justify-between items-start">
                <div><h1 className="text-2xl md:text-3xl font-bold text-gray-800 dark:text-white">Welcome, {currentUser.fullName}!</h1><p className="text-gray-500 dark:text-gray-400 mt-1">Here's a summary of your account activity.</p></div>
                {currentUser.country && (<div className="hidden sm:flex items-center space-x-2 bg-white dark:bg-gray-800 px-4 py-2 rounded-full text-sm font-semibold text-gray-700 dark:text-gray-200 border border-gray-200 dark:border-gray-700 shadow-sm"><MapPinIcon /><span>{currentUser.country}</span></div>)}
            </div>
            <div className="relative">
                <button onClick={() => setShowCustomize(!showCustomize)} className="absolute top-0 right-0 -mt-8 py-1.5 px-3 text-sm font-bold rounded-md bg-gray-200 text-gray-800 hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600 focus:ring-gray-500">Customize</button>
                {showCustomize && (<div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-lg mb-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2 text-sm">{Object.keys(visibleWidgets).map(key => (<label key={key} className="flex items-center space-x-2"><input type="checkbox" checked={visibleWidgets[key as keyof typeof visibleWidgets]} onChange={() => toggleWidget(key as keyof typeof visibleWidgets)} className="rounded"/><span className="capitalize">{key.replace(/([A-Z])/g, ' $1')}</span></label>))}</div>)}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <ShareButtons url={referralLink} title="Join me on SmartEarning and start earning today!" />
                 {visibleWidgets.breakdown && <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6"><h3 className="font-semibold mb-3 text-gray-800 dark:text-white text-center">Referral Commission Breakdown</h3><PieChart currency={currentUser.currency} data={[{ label: 'Direct', value: stats.directCommission, color: '#3b82f6' },{ label: 'Indirect', value: stats.indirectCommission, color: '#8b5cf6' }]} /></div>}
            </div>
