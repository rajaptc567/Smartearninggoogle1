...
// @desc    Get withdrawals (Role-Aware)
...
export const getWithdrawal = async (req, res) => {
    try {
        ...
        // Security check
        if (req.user.role !== 'admin' && req.user.role !== 'superadmin' && String(withdrawal.userId) !== String(req.user.id)) {
    ...
    try {
        ...
        if (settings.withdrawalFrequency && settings.withdrawalFrequency.enabled) {
            ...
            if (lastWithdrawal) {
                ...
                if (now < lastDate + durationMs) return res.status(400).json({ success: false, error: `Withdrawal frequency limit reached.` });
            }
        }

        if (user.walletBalance < amount) return res.status(400).json({ success: false, error: 'Insufficient balance' });

        const updatedUser = await User.findByIdAndUpdate(userId, { $inc: { walletBalance: -Number(amount.toFixed(2)) } }, { new: true });
        const withdrawal = await Withdrawal.create({ ...req.body, currency: user.currency });
        ...
        res.status(201).json({ success: true, data: { withdrawal, user: updatedUser } });
...
export const updateWithdrawal = async (req, res) => {
    try {
        ...
        if (status === 'Matching') {
            ...
            if (originalStatus === 'Matching') await PaymentMethod.findOneAndUpdate({ p2pWithdrawalId: withdrawal._id }, methodData);
            else {
        ...
        if (originalStatus === 'Matching' && status !== 'Matching') await PaymentMethod.deleteOne({ p2pWithdrawalId: withdrawal._id });
        ...
        const originalTransaction = await Transaction.findOne({ userId: withdrawal.userId, type: 'Withdrawal Request', description: { $regex: `Withdrawal #${withdrawal._id}` } });
        let updatedUser;
        if ((originalStatus === 'Pending' || originalStatus === 'Matching') && status === 'Rejected') {
            ...
            await Transaction.create({
                ...
                type: 'Withdrawal Refund', amount: withdrawal.amount, status: 'Approved', description: `Refund for rejected withdrawal #${withdrawal._id}`
            });
            if (originalTransaction) { originalTransaction.status = 'Rejected'; await originalTransaction.save(); }
        } else { updatedUser = await User.findById(withdrawal.userId); }
        
        if (status === 'Paid' || status === 'Approved') {
            if (originalTransaction) { originalTransaction.status = status === 'Paid' ? 'Approved' : status; await originalTransaction.save(); }
            await Notification.create({ userId: withdrawal.userId, message: `Your withdrawal for ${withdrawal.currency}${withdrawal.finalAmount.toFixed(2)} has been ${status === 'Paid' ? 'successfully paid' : 'approved'}.` });
        }
