...
export const getTransfers = async (req, res) => {
    try {
        ...
        if (req.user.role !== 'admin' && req.user.role !== 'superadmin') {
            filter = { $or: [{ senderId: req.user.id }, { recipientId: req.user.id }] };
        }
...
export const createTransfer = async (req, res) => {
    ...
    try {
        ...
        if (!sender || !recipient) return res.status(404).json({ success: false, error: 'Sender or recipient not found.' });
        ...
        if (sender.currency !== recipient.currency && !config.allowCrossCurrency) return res.status(403).json({ success: false, error: 'Cross-currency transfers are currently disabled.' });

        const tier = config.tiers.find(t => t.currency === sender.currency && amount >= t.minAmount && amount <= t.maxAmount && (t.enabled === undefined || t.enabled === true));
        if (!tier) return res.status(400).json({ success: false, error: `Transfer amount not within allowed limits.` });

        let fee = Number((tier.feeType === 'percentage' ? (amount * tier.feeValue) / 100 : tier.feeValue).toFixed(2));
        const totalDeduction = Number((amount + fee).toFixed(2));

        if (sender.walletBalance < totalDeduction) return res.status(400).json({ success: false, error: `Insufficient funds.` });

        const updatedSender = await User.findByIdAndUpdate(senderId, { $inc: { walletBalance: -totalDeduction } }, { new: true });
        const transfer = await Transfer.create({ ...req.body, currency: sender.currency, fee: fee, totalDeducted: totalDeduction, status: 'Pending' });
        ...
        await Notification.create({ userId: sender._id, message: `Your transfer of ${sender.currency}${amount.toFixed(2)} to ${recipient.username} is pending approval.` });
...
export const updateTransfer = async (req, res) => {
    ...
    try {
        ...
        if (!transfer || transfer.status !== 'Pending') return res.status(400).json({ success: false, error: 'Transfer not found or already processed.' });

        const sender = await User.findById(transfer.senderId);
        const recipient = await User.findById(transfer.recipientId);
        const originalTransaction = await Transaction.findOne({ userId: transfer.senderId, type: 'Transfer Request', description: { $regex: `Transfer .* #${transfer._id}` } });
        ...
        if (status === 'Approved') {
            ...
            if (originalTransaction) { originalTransaction.status = 'Approved'; await originalTransaction.save(); }
            await Transaction.create({
                userId: recipient._id, userName: recipient.username, currency: recipient.currency,
                type: 'Transfer Received', amount: receivedAmount, description: `Received from ${sender.username}`, sourceUserId: sender._id, status: 'Approved'
            });
            ...
            if (originalTransaction) { originalTransaction.status = 'Rejected'; await originalTransaction.save(); }
            await Transaction.create({
                userId: sender._id, userName: sender.username, currency: sender.currency,
                type: 'Transfer Refund', amount: refundAmount, status: 'Approved', description: `Refund for rejected transfer #${transfer._id}`
            });
