...
const executePlanPurchase = async (user, plan, isAuto = false, settings, exchangeRates, defaultRates, allPlans) => {
    ...
    // Check for held commissions that this new plan might unlock (excluding upgrade-reserved ones)
    ...
    if (heldCommissions.length > 0) {
        ...
        for (const comm of heldCommissions) {
            // Check if transaction is a "Hold Position" - we DON'T release those as they belong to heldBalance reservoir
...
// ... createUser, loginUser, getUsers, getUser remain the same ...
...
// ... updateUser, bulkUpdateRestrictions, deleteUser, bulkDeleteUsers, adjustWallet remain same ...
...
// ... reset handlers remain unchanged ...
